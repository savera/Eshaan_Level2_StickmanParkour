...
package jaco.mp3.player.resources.converter;
...
import jaco.mp3.player.resources.Obuffer;
...
public class Converter
{
		...
		throws JavaLayerException
	{
		...
		try {
			...
			SoundStream stream = new SoundStream(sourceStream);
				...
				for (; frame<frameCount; frame++)
				{
					try
					{
						...
						if (output==null)
						{
							...
							int channels = (header.mode()==Frame.SINGLE_CHANNEL) ? 1 : 2;
	...
	static public interface ProgressListener
	{
		 ...
		 */
		public void parsedFrame(int frameNo, Frame header);
		 ...
		 */
		public void readFrame(int frameNo, Frame header);
		 ...
		 */
		public void decodedFrame(int frameNo, Frame header, Obuffer o);
